#include "stdafx.h"
#include "..\Public\DefaultDoor.h"
#include "LoadDatFiles.h"
#include "PositionManager.h"

CDefaultDoor::CDefaultDoor(_pGraphicDevice * pGraphicDevice, _pContextDevice * pContextDevice)
...
HRESULT CDefaultDoor::NativeConstruct(OBJ_INFO tObjInfo, _float fMove, _float fRotate, _bool bX)
{
...
_int CDefaultDoor::Tick(_double dTimeDelta)
{
...
_int CDefaultDoor::LateTick(_double dTimeDelta)
{
	CGameInstance* pGameInstance = CGameInstance::Get_Instance();
	m_bRender = false;
...
void CDefaultDoor::Interaction()
{
	...
	*/
	...
	if (m_bAction)
	{
		...
		else if (m_fRotate != 0 && m_fMove == 0)
		{
			if (!m_bReverse)
				CSoundMgr::Get_Instance()->PlaySound(L"Toilet_Door.wav", CSoundMgr::SLIDEDOOR_ON);
			else if (m_bReverse)
				CSoundMgr::Get_Instance()->PlaySound(L"Toilet_Door.wav", CSoundMgr::SLIDEDOOR_OFF);
...
void CDefaultDoor::Event()
{
	if (m_bAction)
		return;
	CPositionManager::Get_Instance()->Add_Time(5.0);
	m_bAction = true;
}

void CDefaultDoor::Collect_Event()
{
	CGameInstance* pGameInstance = CGameInstance::Get_Instance();
	...
	if (CLoadDatFiles::Get_Instance()->Get_ModelLoading())
	{
		if (m_pCollider->Collision_OBB(pGameInstance->Get_Pivot(), pGameInstance->Get_Ray(), fDistance))
		{
			...
			tDesc.eState = CPickingEventManager::FSM_DOOR;
			pGameInstance->Add_Event(tDesc);
		}
	}
}

void CDefaultDoor::DoorLeftX()
{
	//1_ DoorLeftX - Move X Position - 0.66
...
void CDefaultDoor::DoorRotationPlus()
{
	//5_ DoorRotationP - Rotate Y Axis 90 Degree.
	if (m_iAcc != 90 && !m_bReverse)
	{
...
void CDefaultDoor::DoorRotationMinus()
{
	//6_ DoorRotationM - Rotate Y Axis -90 Degree.
	if (m_iAcc != -90 && !m_bReverse)
	{
...
CDefaultDoor * CDefaultDoor::Create(_pGraphicDevice * pGraphicDevice, _pContextDevice * pContextDevice, OBJ_INFO tObjInfo, _float fMove, _float fRotate, _bool bX)
{
	CDefaultDoor* pInstance = new CDefaultDoor(pGraphicDevice, pContextDevice);
	if (FAILED(pInstance->NativeConstruct(tObjInfo, fMove, fRotate, bX)))
...
void CDefaultDoor::Compute_Distance()
{
	CGameInstance* pGameInstance = CGameInstance::Get_Instance();
	_vector vDistance;
