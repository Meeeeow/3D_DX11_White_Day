                     ...
                     _In_ int       nCmdShow)
{
    ...
    while (true)
    {
		...
		dTimeAcc += pGameInstance->Compute_Timer(TEXT("Timer_Default"));

		if (dTimeAcc >= 1.0 / 60.0)
		{
			dTimeAcc = 0.0;

			_double		TimeDelta = pGameInstance->Compute_Timer(TEXT("Timer_60"));
			pMainApp->Tick(TimeDelta > 0.1f ? (1.f / 60.0f) : TimeDelta);
			pMainApp->Render();
		}
    }
